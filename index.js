...
function generateBMFont(fontPath, opt, callback) {
    ...
    let charset = (opt.charset =
        (typeof opt.charset === 'string' ? Array.from(opt.charset) : opt.charset) || reuse.charset || defaultCharset);
    ...
    const font = typeof fontPath === 'string' ? opentype.loadSync(fontPath) : opentype.parse(utils.bufferToArrayBuffer(fontPath));
        ...
        const container = {
            data: {
                ...
                fontData: {
                    ...
                    width: width,
                    height: height,
                    xoffset: roundDecimal == null ? Math.round(bBox.x1) - pad : utils.roundNumber(bBox.x1 - pad, roundDecimal),
                    yoffset:
                        roundDecimal == null
